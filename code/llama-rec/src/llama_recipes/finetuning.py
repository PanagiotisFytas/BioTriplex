...
from llama_recipes.utils.bidirectional_models import LlamaBiForMNTP
from llama_recipes.utils.embedding_models import LlamaWithEmbeddings
...
def main(**kwargs):
    ...
    if config.model_type == "mllama":
        ...
        is_vision = False
        if train_config.bidirectional_attention_in_entity_tokens:
            print("Bidirectional attention in entity tokens is enabled")
            model = LlamaBiForMNTP.from_pretrained(
                train_config.model_name,
                quantization_config=bnb_config,
                use_cache=use_cache,
                attn_implementation="sdpa" if train_config.use_fast_kernels else None,
        ...
        elif train_config.use_embeddings:
            print("Using embeddings as additional modality")
            model = LlamaWithEmbeddings.from_pretrained(
                train_config.model_name,
                quantization_config=bnb_config,
                use_cache=use_cache,
                attn_implementation="sdpa" if train_config.use_fast_kernels else None,
                ...
                torch_dtype=torch.float16 if train_config.use_fp16 else torch.bfloat16,
                embedding_dim=train_config.embedding_dim if hasattr(train_config, 'embedding_dim') else 768,
                pooling_type=train_config.pooling_type if hasattr(train_config, 'pooling_type') else "mean",
            )
        else:
    ...
    if train_config.use_entity_tokens_as_targets and train_config.entity_special_tokens:
