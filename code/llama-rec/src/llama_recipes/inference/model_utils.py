...
from llama_recipes.utils.bidirectional_models import LlamaBiForMNTP, BiPeftModel



# Function to load the main model for text generation
def load_model(model_name, quantization, use_fast_kernels, bidirectional=False, **kwargs):
...
def load_peft_model(model, peft_model, bidirectional=False):
    if bidirectional:
        peft_model = BiPeftModel.from_pretrained(model, peft_model)
        return peft_model
...
def load_llama_from_config(config_path, bidirectional=False):
    if bidirectional:
        model_config = LlamaConfig.from_pretrained(config_path)
        model = LlamaBiForMNTP(config=model_config)
        return model
